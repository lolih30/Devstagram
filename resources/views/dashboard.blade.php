...
@section('contenido')
    ...
    <div class="flex justify-center">
        <div class="w-full sm:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row ">
            ...
            <div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center md:items-start md:justify-center py-10 ">
                ...
                @auth
                    {{-- si el id de usuario actual es diferente del que esta autenticado --}}
                    @if ($user->id !== auth()->user()->id)
                        @if (!$user->siguiendo(auth()->user()))
                            <form method="POST" {{--   usuario que estamos visitando en el perfil --}} action=" {{ route('users.follow', $user) }} ">
                                @csrf
